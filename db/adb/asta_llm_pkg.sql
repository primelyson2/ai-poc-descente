...
CREATE OR REPLACE PACKAGE BODY asta_llm_pkg AS
  ...
  BEGIN
    ...
    clob_app(l_prompt, 'Legacy outer-join compatibility preflight is mandatory: if the query block where a helper would be joined contains any (+) predicate, add no ANSI JOIN to that block. Instead, put the original immediate consumer (for example XX) and its helper ANSI LEFT JOINs in a new nested inline view that projects the consumer''s original columns plus only the replacement aggregate values; leave the parent comma-separated sources and every (+) predicate verbatim. Return no candidate if this syntax isolation cannot be completed.' || CHR(10));
