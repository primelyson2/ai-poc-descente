...
CREATE OR REPLACE PACKAGE BODY asta_llm_pkg AS
  ...
  BEGIN
    ...
    clob_app(l_prompt, 'Correlated-aggregate lift safety is mandatory: build each helper CTE only from the original inner tables, grouped by every original correlation key, and LEFT JOIN it from the original immediate outer consumer. Never join the helper producer to a pre-existing CTE as a substitute for that outer consumer, and do not move an outer-alias DECODE or other wildcard correlation into the helper unless its exact wildcard semantics are preserved; return no candidate when that localized lift is not possible.' || CHR(10));
