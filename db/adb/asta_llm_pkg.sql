...
CREATE OR REPLACE PACKAGE BODY asta_llm_pkg AS
  ...
  BEGIN
    ...
    IF NVL(DBMS_LOB.GETLENGTH(p_sql), 0) >= 12000 THEN
      clob_app(l_prompt, 'Long-SQL rewrite boundary: target exactly one repeated-access pattern. You may add one helper CTE and replace every occurrence of that same logical computation in the required producer and consumer query blocks; this still counts as one localized structural change. Copy every unrelated query block, CTE, UNION ALL branch, join, predicate, and select-list expression verbatim from the input. Do not decompose or rebuild the full statement into a new CTE architecture. In particular, do not change any UNION ALL branch projection count or reference a grouping key after it has been removed by aggregation.' || CHR(10));
    END IF;
